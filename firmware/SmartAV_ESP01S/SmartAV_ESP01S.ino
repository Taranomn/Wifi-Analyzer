...
uint32_t lastHubDebugAt = 0;
uint32_t lastHubAutoJoinScanAt = 0;
...
#else
  const String ssid = hubSsid();
  const bool ok = WiFi.softAP(ssid.c_str(), kHubPassword, 1, false, 4);
  Serial.printf("Hub AP start %s: %s at %s\n",
                ok ? "ok" : "failed",
                ssid.c_str(),
                WiFi.softAPIP().toString().c_str());
#endif
...
#if defined(ESP8266)
bool isAutoHubSsid(const String &ssid) {
  return ssid.startsWith("SmartAV-Hub-");
}

void scanAndJoinHubIfAvailable() {
  if (connectionPending || WiFi.status() == WL_CONNECTED) return;
  if (!savedSsid.isEmpty() && !isAutoHubSsid(savedSsid)) return;
  if (millis() - lastHubAutoJoinScanAt < 10000) return;
  lastHubAutoJoinScanAt = millis();

  Serial.println("ESP-01S hub scan: looking for SmartAV-Hub-*");
  const int count = WiFi.scanNetworks(false, true);
  String bestSsid;
  int bestRssi = -127;
  for (int i = 0; i < count; ++i) {
    const String ssid = WiFi.SSID(i);
    if (isAutoHubSsid(ssid) && WiFi.RSSI(i) > bestRssi) {
      bestSsid = ssid;
  ...
  if (bestSsid.isEmpty()) {
    Serial.printf("ESP-01S hub scan: no SmartAV hub found (%d networks scanned)\n", count);
    return;
  }

  Serial.printf("ESP-01S hub scan: joining %s rssi=%d\n", bestSsid.c_str(), bestRssi);
  beginConnection(bestSsid, kHubPassword);
}
#endif
...
void handleHubRegister() {
  ...
  if (index >= 12) {
    Serial.println("Hub register rejected: child registry full");
  ...
  Serial.printf("Hub child registered: node=%s ip=%s ssid=%s rssi=%d channel=%d loss=%.1f ping=%.1f children=%u\n",
                child.node.c_str(),
                child.ip.c_str(),
                child.ssid.c_str(),
                child.rssi,
                child.channel,
                child.packetLossPercent,
                child.averagePingMs,
                static_cast<unsigned>(hubChildCount));
  server.send(200, "application/json", R"({"ok":true})");
#endif
}

#if !defined(ESP8266)
void printHubDebugStatus() {
  Serial.printf("Hub status: ap_ssid=%s ap_ip=%s ap_clients=%u registered_children=%u sta_status=%d sta_ssid=%s sta_ip=%s\n",
                hubSsid().c_str(),
                WiFi.softAPIP().toString().c_str(),
  ...
  for (size_t i = 0; i < hubChildCount; ++i) {
    const HubChild &child = hubChildren[i];
    Serial.printf("  child[%u]: node=%s ip=%s age=%lus connected=%s rssi=%d ping=%.1f loss=%.1f\n",
                  static_cast<unsigned>(i),
                  child.node.c_str(),
                  child.ip.c_str(),
                  static_cast<unsigned long>((millis() - child.lastSeen) / 1000),
...
#else
  if (millis() - lastHubDebugAt > 5000) {
    lastHubDebugAt = millis();
    printHubDebugStatus();
  }
#endif
